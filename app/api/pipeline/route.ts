...
Return ONLY valid JSON:
{
  ...
  "ai_noise_factor": "<low|medium|high>",
  "ai_noise_detail": "<1-2 sentences on how AI noise specifically affects this field>",
  "skill_atrophy_risk": "<low|medium|high>",
  "skill_atrophy_detail": "<1-2 sentences explaining which skills are at risk>",
  "hiring_impact": "<how AI is changing hiring in this field>",
  "regulatory_friction": "<any regulatory uncertainty AI is creating>",
  "silver_lining": "<one genuine opportunity these bottlenecks create>",
...
export async function POST(req: Request) {
  ...
  // Use provided API key, fall back to server env var
  const resolvedKey = apiKey || process.env.ANTHROPIC_API_KEY;
  if (!resolvedKey) {
    return new Response("Missing API key — provide one in the UI or set ANTHROPIC_API_KEY on the server", { status: 401 });
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (const agent of agents) {
        // Frontend expects "start" to mark agent as running
        ...
        try {
            ...
            (async () => {
              for await (const event of claudeStream) {
                if (
                  ...
                  fullText += chunk;
                  // Frontend expects "stream" for token-by-token text
                  send({ type: "stream", agentId: agent.id, chunk });
                }
              }
            })(),
            timeout,
          ...
          // Frontend expects "data" to mark agent as complete
          send({ type: "data", agentId: agent.id, data });
        } catch (err) {
          send({ type: "error", agentId: agent.id, message: String(err) });
        }
      }
